...
import { categoryList } from '../../utlis/context';
...
const { handleSubmit, errors } = useForm({
  validationSchema: yup.object({
    ...
    unit: yup.string(),
    originPrice: yup.number().min(1).required(),
    price: yup.number(),
...
const { value: originPrice } = useField('originPrice');
...
const isOriginPriceError = computed(() => Object.hasOwn(errors.value, 'originPrice'));
...
<template>
  <Modal :no-scroll="noScroll" :showModal="showModal" @closing="$emit('closing')">
    <template v-slot:content>
      <div class="w-[768px] p-5">
        <div class="mb-6 flex w-full items-center justify-between">
          <h4 class="font-bold">新增產品</h4>
          <span
            class="material-icons-outlined icon-hover text-cc-other-2"
            @click="$emit('closing')"
          >
            close
          </span>
